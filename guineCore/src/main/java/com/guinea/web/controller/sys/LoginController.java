...
public class LoginController extends BaseFormController {
    ...
    public String showSysLogin(HttpServletRequest request, ModelMap model) {
        String  flag_v =WebUtil.checkBrowser(request);
        model.put("path", request.getContextPath());
        model.put("flag_v",flag_v);
        return "login";
    }
    ...
    public ModelAndView main_index(HttpServletRequest request) {
        ...
        StringBuffer datebuffer = new StringBuffer(dateTime.toString("yyyy年MM月dd日"));
        datebuffer.append(" ");
        datebuffer.append(JodatimeUtil.getWeekDay(dateTime));
        modelAndView.addObject("path", path);
        modelAndView.addObject("dateShow", datebuffer);
    ...
    public String quickMenu(ModelMap model, HttpServletRequest request) {
        ...
        //ws://skyjasper.eicp.net:45811/ws
        String web_socket_url = "ws://" + Constents.web_socket_conn + "/ws";
        Long userId = user.getId();
        model.put("path", request.getContextPath());
        model.put("web_socket_url", web_socket_url);
